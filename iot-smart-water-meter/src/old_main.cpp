  ...
  while (true)
  {
    ...
    if (current_user_uid != "00000000")
    {
      ...
      if (currentUser.userName != oldusername)
      {
        ...
        oldusername = currentUser.userName; // Update old username
      }
      if (currentUser.waterUsage != oldwaterusage)
      {
        Blynk.virtualWrite(V2, currentUser.waterUsage);
        oldwaterusage = currentUser.waterUsage; // Update old water usage
      }
      if (totalWaterUsed != oldwaterused)
      {
        Blynk.virtualWrite(V3, totalWaterUsed);
        oldwaterused = totalWaterUsed; // Update old water used
      }
      if (currentUser.balance != oldbalance)
      {
        Blynk.virtualWrite(V4, currentUser.balance);
        oldbalance = currentUser.balance; // Update old balance
      }
    }
    else
    {
      ...
      if ((oldusername != "NO AUTHORIZED USER"))
      {
        ...
        oldusername = "NO AUTHORIZED USER";
        oldwaterused = 0.0;
        oldwaterusage = 0.0;
        oldbalance = 0.0;
        // Reset Blynk virtual pins to default values.
        Blynk.virtualWrite(V1, "NO AUTHORIZED USER");
...
void setup()
{
  ...
  xTaskCreate(checkConnection, "CheckConnectionTask", 8096, NULL, 1, &checkConnectionTaskHandle);
  xTaskCreate(blynkTask, "BlynkTask", 8096, NULL, 3, NULL);
}
